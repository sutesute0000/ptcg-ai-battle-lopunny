...
ENERGY_IDS = {WATER, MIST, ENRICHING, SPIKY}
BASIC_POKE = {BUNEARY, DUNSPARCE, FAN_ROTOM, SNORUNT}

# Two attacker lines now. Lopunny wants any energy (Gale Thrust is colorless);
# Froslass's Resentful Refrain costs {W} specifically, so the three Basic Water
# in this list are its and nothing else's.
MEGA_IDS = {LOPUNNY, FROSLASS}
PREEVO_OF = {BUNEARY: LOPUNNY, SNORUNT: FROSLASS}
ATTACKER_LINE = {BUNEARY, LOPUNNY, SNORUNT, FROSLASS}
...
class Policy:
    ...
    def fueled_bench_lopunny(self):
        return [b for b in self.bench if b.id in MEGA_IDS and b.energies]
    ...
    def lopunny_line_on_board(self):
        return sum(self.board_count(c) for c in ATTACKER_LINE)
    ...
    def score_main(self, o) -> float:
        ...
        if t == OptionType.EVOLVE:
            src = self.opt_card(o)
            if src is not None and src.id in MEGA_IDS:  # evolving INTO a Mega
                return 9000
            return 8900
        ...
        if t == OptionType.ATTACH:
            ...
            if (is_active_tgt and self.active is not None
                ...
                return 9200
            if tgt.id == FROSLASS or tgt.id == SNORUNT:
                # Resentful Refrain costs {W}; nothing else in the list pays it,
                # and a Water on a Lopunny is a wasted card.
                if src_id != WATER:
                    return 120
                if not tgt.energies:
                    return 8620 if tgt.id == FROSLASS else 6400
                return 200
            if src_id == WATER and tgt.id in (LOPUNNY, BUNEARY):
                return 130          # save the three Waters for the Froslass line
    ...
    def play_score(self, cid) -> float:
        ...
        if cid in BASIC_POKE:
            if cid in PREEVO_OF and self.lopunny_line_on_board() < 3:
                return 8800
            if cid == DUNSPARCE:
        ...
        if cid == POFFIN:
            ...
            return 1000
        if cid == HAND_TRIMMER:
            # Both players discard down to 5. Against Alakazam that is the
            # whole matchup: Powerful Hand is 20 damage per card in THEIR hand,
            # so trimming them to 5 caps a 300-point swing at 100. Only worth a
            # card when they are actually holding a hand worth cutting.
            if self.opp.handCount >= 8:
                return 7900
            if self.opp.handCount >= 7 and len(hand) <= 6:
                return 6400
            return 800
        if cid == BATTLE_CAGE:
            # Stops damage counters landing on our Bench — that is Dragapult's
            # Phantom Dive spread, which otherwise picks off the 70 HP pieces
            # the loop is built on.
            return 6900 if not self.st.stadiumPlayed else 200
        if cid == AIR_BALLOON:
    ...
    def attack_damage(self, atk) -> float:
        """Printed damage is 0 for attacks that scale, so read the board.

        Resentful Refrain does 50 per card in the OPPONENT's hand: with a
        normal seven-card hand that is 350, more than Gale Thrust's 230, off a
        single Water energy. Taking the printed 0 at face value would make the
        agent never attack with Froslass at all."""
        dmg = atk.damage or 0
        text = (atk.text or '').lower()
        if not dmg and 'for each card in your opponent' in text:
            per = 50 if '50 damage' in text else 20
            dmg = per * self.opp.handCount
        return dmg

    def most_damaged_mega(self):
        megas = [p for p in [self.active] + list(self.bench)
                 if p is not None and p.id in MEGA_IDS and p.hp < p.maxHp]
    ...
    def shield_rank(self, o) -> float:
        ...
        base = {DUNSPARCE: 100, FAN_ROTOM: 60, DUDUNSPARCE: 55,
                BUNEARY: 40, SNORUNT: 40}.get(p.id, 50)
        return base + (40 if can_flee else 0) + p.hp / 20

    def setup_rank(self, o) -> float:
        c = self.opt_card(o)
        cid = c.id if c is not None else -1
        return {BUNEARY: 100, SNORUNT: 100, DUNSPARCE: 60, FAN_ROTOM: 40}.get(cid, 10)
